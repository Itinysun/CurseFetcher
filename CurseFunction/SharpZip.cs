...
namespace CurseFunction
{
    public class SharpZip
    {
        ...
        public static string DecomparessFile(string sourceFile, string destinationDirectory = null, string password = null)
        {
            ...
            try
            {
                ...
                using (ZipInputStream zipStream = new ZipInputStream(File.Open(sourceFile, FileMode.Open,FileAccess.Read, FileShare.Read)))
                {
                    ...
                    while (zipEntry != null)
                    {
                        ...
                        {
                            //创建路径
                            var destPath = Path.Combine(destinationDirectory, Path.GetDirectoryName(zipEntry.Name));
                            if (!Directory.Exists(destPath))
                                Directory.CreateDirectory(destPath);

                            //提取根目录
                            var zRoot = CommonFunction.GetRootDir(zipEntry.Name);
                            if(!top.Contains(zRoot))
                                top.Add(zRoot);
            ...
            {
                Settings.Xlog.Add(ex.Message, "解压文件发生错误!", XLog.AsyncLogLevel.FATAL);
                return null;
            }
        }
        #endregion
